-- submit_curriculum_import: target_language as parameter, not hardcoded.
--
-- The prior version hardcoded decks.target_language = 'en'. The wizard path
-- writes the canonical full-word form ('English'), so curriculum-imported
-- decks landed in a separate raw-ISO bucket on the Decks page pill row.
--
-- Caller (frontend bridge) now passes the language sourced from canonical
-- curriculum content (curriculum/content/<iso>/*.json target_language field),
-- mapped through LANGUAGES[].code → LANGUAGES[].value at the bridge boundary,
-- so the value written here matches the wizard convention.
--
-- Signature change requires drop-then-create; CREATE OR REPLACE cannot alter
-- a function's argument list.
--
-- Design ref: investigations/CURRICULUM_TARGET_LANGUAGE_DESIGN.md §7 (Option B2).

begin;

drop function if exists public.submit_curriculum_import(text, integer, text, jsonb);
...
comment on function public.submit_curriculum_import(text, integer, text, jsonb, text) is
  'Materializes a curriculum level as a card deck owned by the calling learner. Idempotent on (user, category_slug, level_number). Target language passed as a parameter from the bridge, sourced from canonical curriculum content. Does not debit credits.';
