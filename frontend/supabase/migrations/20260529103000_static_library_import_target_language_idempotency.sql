-- Static library imports can materialize the same category level in multiple
-- target languages. Keep the existing no-credit curriculum import RPC, but
-- scope idempotency to target_language so English Animals Level 1 and German
-- Tiere Level 1 are separate ready-made card decks.
...
comment on function public.submit_curriculum_import(text, integer, text, jsonb, text) is
  'Materializes a static/curriculum level as a card deck owned by the calling learner. Idempotent on (user, category_slug, level_number, target_language). Does not debit credits.';
