-- Preserve the user's literal word input before enrichment can normalize it.

alter table public.words
  add column if not exists original_input text;

-- Best historical approximation: pre-existing rows may already have had
-- words.word overwritten with word_target for non-phrase inputs, so the
-- literal user input is unrecoverable for those rows.
update public.words
   set original_input = word
 where original_input is null;
...
comment on function public.submit_generation(jsonb, text[], jsonb, uuid, text) is
  'Atomic user generation submit: validates ownership and deck-type/card-tier pricing, creates deck/job/owned words, records exact credits charged, and debits credits.';
