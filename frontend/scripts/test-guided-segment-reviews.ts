...
const journeyCssSource = readSource('../src/components/today/TodayJourney.css')
const overviewMastheadSource = sliceBetween(overviewSource, '<header className="today-journey-masthead">', '</header>')
const segmentReviewTileSource = sliceBetween(overviewSource, 'function SegmentReviewTile', 'function SegmentTrophyTile')

assert('Today masthead no longer renders the visible Path Check button', !overviewMastheadSource.includes("today.path.pathCheck") && !overviewMastheadSource.includes('PathCheckTile'))
assert('Today still passes Path Check href for the separate path-level diagnostic', todaySource.includes('pathCheckHref=') && todaySource.includes('mode=path-check'))
...
assert('review nodes do not reuse normal lesson-card styling as their primary hook', segmentReviewTileSource.includes('today-journey-reward') && !segmentReviewTileSource.includes('today-journey-lesson'))
...
assert('review tiles are always rendered as clickable links', segmentReviewTileSource.includes('<Link') && !segmentReviewTileSource.includes('if (!isAvailable)'))
assert('review tiles no longer render visible progress text', !segmentReviewTileSource.includes('/5'))
assert('review tiles no longer render visible lock or not-ready state', !containsAny(segmentReviewTileSource, ['aria-disabled', 'today.path.notReadyYet', '<Lock', 'today.path.startReview']))
assert('review control keeps its localized label and lesson range as real text', segmentReviewTileSource.includes('<strong>{label}</strong>') && segmentReviewTileSource.includes('rangeLabel'))
assert('review completion treatment follows the saved segment review state', overviewSource.includes('readGuidedSegmentReviewRecord') && segmentReviewTileSource.includes("isReviewComplete ? 'complete' : 'upcoming'") && segmentReviewTileSource.includes('data-review-complete'))
assert('review tiles no longer reference PNG review assets', !overviewSource.includes('-review.png'))
assert('journey CSS defines separated review controls outside the lesson rail', journeyCssSource.includes('.today-journey-rewards') && journeyCssSource.includes('.today-journey-reward'))
assert('review label keeps a compact accessible control while its icon remains bounded', segmentReviewTileSource.includes('aria-label={accessibleLabel}') && journeyCssSource.includes('width: 2.75rem') && journeyCssSource.includes('.today-journey-reward strong'))
...
assert('A1 Practical 8 Segment Review is available with no completed lessons in the selected segment/vibe', buildGuidedSegmentReviewPlan(createEmptyTodayProgressState(), pathIds[7]!, 2, 'wistful', fixedRng())?.items.length === 5)
assert('A1 Practical 9 Segment Review is available with no completed lessons in the selected segment/vibe', buildGuidedSegmentReviewPlan(createEmptyTodayProgressState(), pathIds[8]!, 1, 'sharp', fixedRng())?.items.length === 5)
assert('A1 Practical 10 Segment Review is available with no completed lessons in the selected segment/vibe', buildGuidedSegmentReviewPlan(createEmptyTodayProgressState(), pathIds[9]!, 2, 'bright', fixedRng())?.items.length === 5)
...
assert('A1 Practical 8 Segment Review route uses story copy', getGuidedSegmentStory(pathIds[7]!, 1) !== undefined && buildGuidedSegmentReviewPlan(createEmptyTodayProgressState(), pathIds[7]!, 1, 'bright', fixedRng())?.items.length === 5)
assert('A1 Practical 9 Segment Review route uses story copy', getGuidedSegmentStory(pathIds[8]!, 2) !== undefined && buildGuidedSegmentReviewPlan(createEmptyTodayProgressState(), pathIds[8]!, 2, 'wistful', fixedRng())?.items.length === 5)
