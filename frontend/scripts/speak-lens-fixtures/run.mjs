#!/usr/bin/env node
// Speak/Lens fixture harness runner.
//
...
//
// Usage:
//   node scripts/speak-lens-fixtures/run.mjs
//   node scripts/speak-lens-fixtures/run.mjs --no-build --only=10,11
...
function log(msg) {
  process.stdout.write(`[speak-lens] ${msg}\n`)
}
...
main().catch((error) => {
  process.stderr.write(`[speak-lens] fatal: ${error?.stack ?? error}\n`)
  process.exitCode = 1
})
