...
const desktopSegmentGridSource = sliceBetween(todayPathOverviewSource, '<div className="today-path-segmentGrid', '<div className="today-path-desktopRewards')
...
const desktopConnectorCss = sliceBetween(todayCssSource, '.today-path-desktopConnectorSegment {', '}')
...
const mobileTopSessionIconCss = sliceBetween(todayCssSource, '.today-session-titleRow > .today-session-iconBadge:not(.today-session-iconBadge--compact) {', '}')
...
assert('desktop path connectors are interleaved between number cards instead of under them', desktopSegmentGridSource.includes('today-path-desktopConnectorSegment') && segmentGridCss.includes('--today-node-size') && segmentGridCss.includes('grid-template-columns') && desktopConnectorCss.includes('height:') && desktopConnectorCss.includes('z-index: 0'), { desktopSegmentGridSource, segmentGridCss, desktopConnectorCss })
...
assert('mobile trophy reward floats without oval chrome like review', mobileTrophyTileCss.includes('background: transparent') && mobileTrophyTileCss.includes('border-width: 0') && mobileTrophyTileCss.includes('box-shadow: none') && mobileTrophyTileCss.includes('padding: 0'), mobileTrophyTileCss)
...
assert('lesson cards launch the selected lesson from the whole card', lessonPathCardSource.includes('onSelectLesson(lesson.id)') && lessonPathCardSource.includes('onStartLesson(lesson.id)') && !todayPathOverviewSource.includes('function isMobileViewport'))
...
assert('selected lesson panel action is a single Next lesson CTA', recommendedLessonPanelSource.includes("t('today.nextLesson')") && !recommendedLessonPanelSource.includes('getActionLabel'), recommendedLessonPanelSource)
...
assert('lesson cards keep only compact numeric visual content', lessonPathCardSource.includes('today-path-cardMarker') && lessonPathCardSource.includes('sr-only') && !lessonPathCardSource.includes('<h3') && !lessonPathCardSource.includes('lesson.title}</h3>'), lessonPathCardSource)
...
assert('path directory owns a collapsible language selector above path choices', pathDirectorySource.includes('languageExpanded') && pathDirectorySource.includes('availableLanguages.map') && pathDirectorySource.includes('setLanguageExpanded(false)'), pathDirectorySource)
assert('path directory owns an optional English-only vibe selector', pathDirectorySource.includes('GuidedVibePicker') && pathDirectorySource.includes("selectedLanguage === 'English'") && pathDirectorySource.includes('getPathVibesAvailable(selectedPathId)') && pathDirectorySource.includes('onSelectVibe={onSelectVibe}'), pathDirectorySource)
...
assert('lesson session hides the duplicate top step icon on mobile only', mobileTopSessionIconCss.includes('display: none'), mobileTopSessionIconCss)
...
assert('Speak recording dot stays outside the start/stop button', guidedSpeechPromptSource.includes('today-speak-recordingDot') && !speakRecordingButtonSource.includes('today-speak-recordingDot'), speakRecordingButtonSource)
assert('Speak recording dot cannot capture pointer events', speakRecordingDotCss.includes('pointer-events: none'), speakRecordingDotCss)
