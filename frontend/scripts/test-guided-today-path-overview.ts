...
assert('future artwork is subdued without fading the opaque backing or lesson number', lessonPathCardSource.includes('data-journey-state={visualState}') && todayJourneyCssSource.includes("[data-journey-state='upcoming'] .today-journey-gem img") && !/\[data-journey-state='upcoming'\] \.today-journey-gem\s*\{[^}]*opacity:/.test(todayJourneyCssSource), lessonPathCardSource)
...
assert('type recall fallback fills the actual input without duplicate answer text or choice chips', typeRecallSource.includes('setAnswer(lesson.typeRecall.answer)') && !containsAny(typeRecallSource, ['today.type.answerLine', 'getGuidedTypeFallbackChoices', 'handleFallbackChoice', "t('today.type.fallbackLabel')", 'theme-chip']))
