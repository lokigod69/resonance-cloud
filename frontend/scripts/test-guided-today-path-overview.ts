...
assert('path overview shows truthful completed progress in the top hero', todayPathOverviewSource.includes('today-path-heroProgressRail') && todayPathOverviewSource.includes('today-path-heroProgressFill') && todayPathOverviewSource.includes("t('today.practice.completedCount'") && todayPathOverviewSource.includes('current: overview.completedCount') && todayPathOverviewSource.includes('total: overview.totalLessons'), todayPathOverviewSource)
...
assert('path overview omits decorative hero and lesson orb image transfers', !todayPathOverviewSource.includes('TODAY_PATH_HERO_ASSET') && !todayPathOverviewSource.includes('TODAY_PATH_LESSON_ORB_ASSET') && !todayPathOverviewSource.includes('today-path-heroOrb') && !todayPathOverviewSource.includes('today-featuredLessonOrb'), todayPathOverviewSource)
...
assert('segment review nodes keep localized copy inside the quiet review artwork', todaySegmentReviewBranchSource.includes('today-path-nodeMedia') && todayPathOverviewSource.includes('today-segment-reviewLabel') && todayPathOverviewSource.includes('{displayLabel}') && todayPathOverviewSource.includes("displayLabel={t('today.step.review')}") && !containsAny(todaySegmentReviewBranchSource, ['today-path-nodeCopy', 'today-path-nodeTitle', 'rangeLabel']), todaySegmentReviewBranchSource)
...
assert('review pill and trophy asset glow as clickable hover affordances', reviewTileHoverCss.includes('box-shadow:') && reviewTileHoverCss.includes('today-segment-reviewBadge') && trophyTileHoverCss.includes('filter:') && trophyTileHoverCss.includes('drop-shadow'), { reviewTileHoverCss, trophyTileHoverCss })
...
assert('path overview keeps selected lesson details inside a glass featured card', todayPathOverviewSource.includes('today-featuredLesson') && todayPathOverviewSource.includes('theme-panel') && todayCssSource.includes('.today-featuredLesson'), todayPathOverviewSource)
...
assert('segment review tiles show localized labels without progress or lock copy', segmentReviewTileSource.includes('today-segment-reviewLabel') && segmentReviewTileSource.includes('{displayLabel}') && !containsAny(segmentReviewTileSource, ['progressLabel', '/5', 'today.path.notReadyYet', 'today.path.startReview', '<Lock']))
...
assert('segment review tiles remove baked-English artwork in favor of the localized pill', segmentReviewTileSource.includes('today-segment-reviewBadge') && segmentReviewTileSource.includes('today-segment-reviewLabel') && !segmentReviewTileSource.includes('/guided/reviews/') && !segmentReviewTileSource.includes('review-complete.webp'))
...
assert('segment review and trophy controls stay compact with a 44px visual review target', todayCssSource.includes('min-height: 2.75rem') && todayCssSource.includes('width: min(100%, 8rem)') && todayCssSource.includes('.today-segment-trophyMedia'))
assert('lesson cards open the tapped lesson through the direct-start handler', lessonPathCardSource.includes('onSelectLesson(lesson.id)') && lessonPathCardSource.includes('onClick={handleLessonTap}') && !todayPathOverviewSource.includes('function isMobileViewport'), lessonPathCardSource)
assert('desktop lesson markers render resolved lesson titles without changing the mobile rail', desktopRouteSource.includes('showDesktopLabel') && lessonPathCardSource.includes('today-path-desktopLessonTitle') && lessonPathCardSource.includes('{title}') && lessonPathCardSource.includes('showRailLabel'), lessonPathCardSource)
...
assert('featured lesson action uses localized Start or Resume from saved draft state', recommendedLessonPanelSource.includes('const isResumable') && recommendedLessonPanelSource.includes("isResumable ? t('today.practice.resume') : t('today.startLesson')") && !recommendedLessonPanelSource.includes('getActionLabel'), recommendedLessonPanelSource)
...
assert('lesson cards keep compact marker content and mobile rows do not repeat the lesson number', lessonPathCardSource.includes('today-path-cardMarker') && lessonPathCardSource.includes('sr-only') && !lessonPathCardSource.includes('today-path-railIndex') && !lessonPathCardSource.includes('<h3') && !lessonPathCardSource.includes('lesson.title}</h3>'), lessonPathCardSource)
...
assert('path header keeps completed progress path-level and leaves selected lesson details to the featured box', todayPathOverviewSource.includes("t('today.practice.completedCount'") && todayPathOverviewSource.includes('current: overview.completedCount') && !todayPathOverviewSource.includes("t('today.path.compactProgress'") && !todayPathOverviewSource.includes('today-path-progressLine'), todayPathOverviewSource)
