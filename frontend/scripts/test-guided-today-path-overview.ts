...
const speakRecordingButtonSource = sliceBetween(speakStepSource, "className={cn(isRecording && 'today-speak-recordingButton')}", '</Button>')
...
assert('Speak step does not render a separate recording status chip', !containsAny(speakStepSource, ['today-speak-recordingStatus', "t('today.speak.recording')"]))
assert('Speak start/stop button keeps separate handlers by recording state', speakStepSource.includes('type="button"') && speakRecordingButtonSource.includes('onClick={isRecording ? handleStop : handleStart}'), speakRecordingButtonSource)
assert('Speak recording dot stays outside the start/stop button', speakStepSource.includes('today-speak-recordingDot') && !speakRecordingButtonSource.includes('today-speak-recordingDot'), speakRecordingButtonSource)
