...
assert('static lessons belong only to active V0 paths', GUIDED_LESSONS.every((lesson) => [pathOneId, pathTwoId, pathThreeId, pathFourId, pathFiveId, pathSixId, pathSevenId, pathEightId, pathNineId, pathTenId, spanishPathOneId, italianPathOneId, italianPathTwoId, italianPathThreeId, frenchPathOneId].includes(lesson.pathId)), GUIDED_LESSONS.map((lesson) => lesson.pathId))
...
assert('path selector source exposes all active paths', JSON.stringify(getGuidedTodayPathOptions().map((path) => path.id)) === JSON.stringify([pathOneId, pathTwoId, pathThreeId, pathFourId, pathFiveId, pathSixId, pathSevenId, pathEightId, pathNineId, pathTenId, spanishPathOneId, italianPathOneId, italianPathTwoId, italianPathThreeId, frenchPathOneId]), getGuidedTodayPathOptions())
