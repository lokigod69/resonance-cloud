...
//
// Usage:
//   node scripts/first-light-fixtures/run.mjs
//   node scripts/first-light-fixtures/run.mjs --no-build --only=10,11
...
function log(msg) {
  process.stdout.write(`[first-light] ${msg}\n`)
}
...
async function launchChrome(pageUrl) {
  ...
  const port = await findOpenPort(9433)
...
async function main() {
  ...
  for (const fixture of fixtures) {
    ...
    try {
      ...
      let done = false
      while (Date.now() < deadline) {
        const state = await evaluate(cdp, 'JSON.stringify({shot: window.__shotRequest ?? null, done: window.__done === true})')
...
main().catch((error) => {
  process.stderr.write(`[first-light] fatal: ${error?.stack ?? error}\n`)
  process.exitCode = 1
})
