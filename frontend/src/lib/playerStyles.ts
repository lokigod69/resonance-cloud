...
export const PLAYER_BAR_CLASS =
  'fixed inset-x-0 bottom-0 z-50 flex flex-wrap items-center gap-x-4 gap-y-0 border-t border-border bg-background/95 px-4 pb-[max(env(safe-area-inset-bottom,0px),0.75rem)] pt-3 shadow-[0_-12px_28px_rgba(0,0,0,0.22)] backdrop-blur-xl supports-[backdrop-filter]:bg-background/90 sm:left-1/2 sm:right-auto sm:bottom-4 sm:w-[calc(100%-3rem)] sm:max-w-5xl sm:-translate-x-1/2 sm:flex-nowrap sm:gap-4 sm:rounded-lg sm:border sm:px-4 sm:py-3 sm:shadow-[0_18px_45px_rgba(0,0,0,0.3)]'
