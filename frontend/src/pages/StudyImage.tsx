...
// Image mode is cloned from the video study presentation (Study.tsx): same media
// frame, reveal and grade flow, but the picture is the primary prompt (no video,
// word hidden until reveal). Recall direction: image → word.
export default function StudyImage() {
  ...
  // Reset the broken-image flag whenever the card changes so a 404 on one word
  // (static-library URLs are not guaranteed to resolve) doesn't strand later cards
  // on the placeholder.
              ...
              <div className="text-center mb-6 px-4 min-h-[3.5rem]">
                {revealed ? (
                  <motion.div
                    ...
                    className="space-y-2"
                  >
                    <h2 className="text-3xl font-bold long-copy">{current.word}</h2>
                    {current.translation && (
