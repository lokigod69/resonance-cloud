...
function getBounds() {
  ...
  return {
    ...
    minY: isMobile ? 12 : 10,
    maxY: isMobile ? 82 : 85,
    spacingX: isMobile ? 20 : 16,
    spacingY: isMobile ? 12 : 10,
    minDistX: isMobile ? 18 : 14,
    minDistY: isMobile ? 11 : 9,
  }
}
...
export default function SyndicateCanvas({
  ...
  const [renderWords, setRenderWords] = useState<SyndicateWordState[]>(() => createWordStates(words, new Set()))
    ...
    function loop() {
      ...
      for (const word of wordStatesRef.current) {
        ...
        if (!word.mastered && !word.decrypting && physicsActive && word.settled <= SETTLED_FRAMES) {
          ...
          word.x = clamp(word.x + word.vx, minX, maxX)
          word.y = clamp(word.y + word.vy, minY, maxY)
        }
...
function RevealModal({
  ...
  return (
    ...
    >
      ...
      <div
        className="relative flex flex-col max-w-lg w-full mx-auto max-h-[85vh] font-mono bg-[#0a0a0a] border border-[#00fff2]/50 p-6 md:p-10 pt-12 syndicate-modal-enter"
