...
def make_french_inventory() -> list[dict[str, Any]]:
    return [
        {
...
def make_portuguese_inventory() -> list[dict[str, Any]]:
    return [
        {
            ...
            "order": 1,
            "concept_id": "animals.dog",
            "target_term": "cachorro",
            "spoken_text": "cachorro",
            "english_qa_label": "dog",
            "part_of_speech": "noun",
            "sense": "animals",
        }
...
def test_french_ready_commit_uses_french_profile_and_lilly_voice(monkeypatch):
    sb = FakeSupabase()
    sb.storage = RecordingStorage()
    sb.table("voices")
    sb.table("language_profiles")
    ...
    sb.table("static_tts_voice_assignments")
    sb._tables["language_profiles"] = [
        {"id": "profile-fr", "language": "French", "name": "French_Test", "is_active": True}
    ]
    sb._tables["voices"] = [
        {
            "id": "voice-row-fr",
            "voice_id": "voice-lilly",
            "name": "Lilly",
            "language": "French",
            "language_code": "fr",
        }
    ...
    async def provider(**kwargs):
        assert kwargs["text"] == "chien"
        assert kwargs["voice_id"] == "voice-lilly"
        assert kwargs["language_code"] == "fr"
        return raw_audio

    monkeypatch.setattr("scripts.generate_static_thematic_tts._probe_duration_ms", lambda _path: 705)

    report = run_inventory(
        sb=sb,
        inventory=make_french_inventory(),
        config=make_config(
            target_language="fr",
            voice_profile_key="static_thematic_fr_lilly_raw_v1",
            profile_name="French_Test",
            voice_name="Lilly",
            commit_db=True,
            allow_provider_calls=True,
            postprocess_mode="raw",
    ...
    assert report["totals"]["generated"] == 1
    assert report["voice_profile"]["resolved_profile_name"] == "French_Test"
    assert report["voice_profile"]["resolved_voice_name"] == "Lilly"
    assert report["voice_profile"]["resolved_voice_language_code"] == "fr"
    assert sb.storage.uploads[0]["path"] == (
        "static/v1/fr/static_thematic_fr_lilly_raw_v1/animals/level-1/animals.dog.mp3"
    )
    assert sb.storage.uploads[0]["data"] == raw_audio
    assert sb._tables["static_tts_asset_usages"][0]["qa_status"] == "ready"
    assert sb._tables["static_tts_voice_assignments"] == []
    assert report["items"][0]["spoken_text"] == "chien"
    assert report["items"][0]["postprocess_mode"] == "raw"
...
def test_portuguese_ready_commit_uses_portuguese_raquel_voice(monkeypatch):
    sb = FakeSupabase()
    sb.storage = RecordingStorage()
    sb.table("voices")
    sb.table("language_profiles")
    ...
    async def provider(**kwargs):
        assert kwargs["text"] == "cachorro"
        assert kwargs["voice_id"] == "voice-raquel"
        assert kwargs["language_code"] == "pt"
        return raw_audio

    monkeypatch.setattr("scripts.generate_static_thematic_tts._probe_duration_ms", lambda _path: 700)

    report = run_inventory(
        sb=sb,
        inventory=make_portuguese_inventory(),
        config=make_config(
            target_language="pt",
            voice_profile_key="static_thematic_pt_raquel_raw_v1",
            profile_name=None,
            voice_name="Raquel",
            commit_db=True,
            allow_provider_calls=True,
    ...
    assert report["voice_profile"]["resolved_voice_name"] == "Raquel"
    assert report["voice_profile"]["resolved_voice_language_code"] == "pt"
    assert sb.storage.uploads[0]["path"] == (
        "static/v1/pt/static_thematic_pt_raquel_raw_v1/animals/level-1/animals.dog.mp3"
    )
    assert sb.storage.uploads[0]["data"] == raw_audio
    assert sb._tables["static_tts_asset_usages"][0]["qa_status"] == "ready"
    assert sb._tables["static_tts_voice_assignments"] == []
    assert report["items"][0]["spoken_text"] == "cachorro"
    assert report["items"][0]["postprocess_mode"] == "raw"
    assert report["items"][0]["raw_duration_ms"] == 700
    assert report["items"][0]["final_duration_ms"] == 700
