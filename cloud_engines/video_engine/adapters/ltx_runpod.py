...
class LTXRunPodAdapter(VideoProviderAdapter):
    ...
    def validate_settings(self, settings: VideoSettings) -> VideoSettings:
        """Validate RunPod-compatible LTX settings without duration snapping."""
    ...
    def generate(
        ...
        *,
        ...
        with logged_api_call(
            ...
            logger.info("RunPod LTX job submitted: %s", request_id)
            ...
            while True:
                ...
                if remaining <= 0:
                    ...
                    raise TimeoutError(
                        f"RunPod job {request_id} timed out after {int(elapsed)}s polling. "
                ...
                try:
                    ...
                    with httpx.Client(
                        ...
                        poll_response = client.get(
                            f"{endpoint_url}/status/{request_id}",
                            headers=headers,
                        )
                ...
                if status == "FAILED":
                    ...
                    raise RuntimeError(
                        f"RunPod job {request_id} failed: {error_msg}"
                    )

                if status == "CANCELLED":
                    raise RuntimeError(f"RunPod job {request_id} was cancelled")

                if status == "TIMED_OUT":
                    error_msg = status_data.get("error")
                    if error_msg:
                        raise TimeoutError(
                            f"RunPod job {request_id} timed out: {error_msg}"
                        )
                    raise TimeoutError(f"RunPod job {request_id} timed out on RunPod")
            ...
            if "error" in job_output:
                raise RuntimeError(
                    f"GPU worker error on job {request_id}: {job_output['error']}"
                )

            video_base64 = job_output.get("video_base64")
            if not video_base64:
                raise RuntimeError(
                    f"RunPod job {request_id} completed but output missing video_base64"
                )
            ...
            ev.record_response(
                response_body=json.dumps(status_data, ensure_ascii=False),
                request_body=json.dumps(runpod_payload, ensure_ascii=False),
