...
def _movie_direction_block(blacklist: list[str] | None = None) -> str:
    ...
    return (
        ...
        "thinks of the movie, creating an instant mnemonic link.\n\n"
...
def _creative_direction_block(mode: str, blacklist: list[str] | None = None) -> str:
    ...
    blocks = {
        "editorial": (
            "=== EDITORIAL ===\n\n"
            "Your job is to create a clean, intentional image that depicts the word "
            "with the polish of a high-end editorial photograph or magazine spread. "
            "Every element in the frame is there because someone chose to put it there.\n\n"
            "Pick the clearest natural depiction of the word, then place it in a "
            "setting that feels considered. Composition is deliberate. Lighting is "
            "intentional. The frame breathes — there is room around the subject. "
            "Materials look real and chosen. Nothing accidental.\n\n"
            "You have full creative range over what editorial means for THIS word — "
            "a portrait, an object study, a curated still life, an environmental shot. "
            "Pick what fits. Clarity wins over cleverness."
        ),
        "cinematic": (
            "=== CINEMATIC ===\n\n"
            "Your job is to create a single frame that feels like it was pulled from "
            "a film. The viewer should feel weight, atmosphere, and intention — like "
            "a cinematographer made deliberate choices about what is in the frame and "
            "what is not.\n\n"
            "Pick a moment that carries narrative or emotional charge. Apply "
            "cinematographic thinking — composition, lens, light, color treatment, "
            "framing, mise-en-scène — to render the word with cinematic gravity.\n\n"
            "You have full creative range over what cinematic means for THIS word. "
            "A wide horizon with a small lone figure, a tight close-up of a face caught "
            "between expressions, a shaft of light cutting through a dim interior, a "
            "long lens compressing a busy street into stillness — many possibilities. "
            "Pick what fits.\n\n"
            "Avoid generic \"moody\" or \"dramatic\" output for its own sake. The "
            "cinematic feel comes from specific compositional and lighting choices, "
            "not adjective stacking."
        ),
        "provocative": (
            "CREATIVE DIRECTION: VISUALLY ARRESTING\n\n"
            "=== PROVOCATIVE ===\n\n"
            "Your job is to create a novel mental anchor for this vocabulary word by "
            "placing its subject in an unexpected context. The image should be absurd, "
            "edgy, hilarious, macabre, dreamlike, or otherwise off — pick the vibe that "
            "best serves THIS word.\n\n"
            "Ways to invert expectation:\n"
            "- Wrong scale (a moon five times its normal size filling the horizon, an "
            "ant-sized human at a normal table)\n"
            "- Wrong context (a tropical fish swimming through a subway car, a pianist "
            "playing in the middle of a freeway)\n"
            "- Wrong physics (a person walking calmly on the ceiling, a waterfall flowing "
            "upward)\n"
            "- Wrong agency (a parrot piloting a plane, a teapot pouring itself)\n"
            "- Wrong combination (a man with a cow's head conducting a meeting, a forest "
            "growing inside a living room)\n"
            "- Wrong tone (an executive boardroom decorated like a children's birthday "
            "party, a funeral attended by clowns who are taking it very seriously)\n\n"
            "The unexpected element should connect to the word's meaning — play with what "
            "the word evokes, what it depicts, what it implies. The strangeness gives the "
            "foreign-language word a fresh hook the learner doesn't already have. Do not "
            "be weird for its own sake; be weird in a way that comments on the word.\n\n"
            "Describe the impossible thing concretely. Do not name any artist, painter, "
            "photographer, director, or filmmaker."
        ),
        "minimal": (
            "CREATIVE DIRECTION: MINIMAL\n"
            "Create typographic-dominant compositions.\n\n"
            "Guidelines:\n"
...
def _art_style_block(style: str) -> str:
    ...
    if style_lower == "auto":
        return (
            "ART STYLE: AUTO\n"
            "Default to \"photorealistic\" unless the word's meaning is so clearly tied "
            "to a specific illustrated medium that photorealism would feel wrong. "
            "Examples of words where you may pick a different style:\n"
            "- \"anime\" / \"manga\" — pick anime\n"
            "- \"comic\" — pick comic_book\n"
            "- \"pixel\" / \"8-bit\" — pick pixel_art\n"
            "- A specific named artistic tradition the word names directly — pick that "
            "tradition\n\n"
            "For every other word, pick \"photorealistic\". This includes provocative, "
            "cinematic, narrative, environmental, scale, and contextual scenes — "
            "photoreal absurdity hits harder than illustrated absurdity, and consistent "
            "style lets the learner focus on meaning.\n\n"
...
def _word_in_image_block(enabled: bool, word: str, language: str) -> str:
    ...
    if enabled:
        return (
            ...
            f'The word "{word}" must appear as readable text in every scene. '
            "It is NOT an overlay or subtitle — it is part of the scene's physical world.\n\n"
            "The word must be:\n"
            ...
            "=== WORD INTEGRATION GUIDANCE ===\n\n"
            "Because word-in-image rendering is enabled, integrate the vocabulary word "
            "into the scene as a physical, readable element. The exact integration "
            "approach depends on the creative direction:\n\n"
            "- editorial: integrate the word elegantly — printed signage, embossed "
            "surface, magazine-style overlay typography\n"
            "- cinematic: the word appears as part of the drama — a sign in the scene, "
            "projected onto a surface, written by hand within the frame\n"
            "- provocative: the word is part of the absurd composition itself — the "
            "cow's tie reads \"FACE\", the cathedral wall reads \"BÜROKRATIE\", the "
            "parrot's airline ticket reads \"PARROT\"\n"
            "- minimal: the word IS the primary visual subject — bold typographic "
            "treatment, dominant in frame, with environment and material providing the "
            "supporting context\n"
            "- movie: the word integrates into the iconic scene — a prop label, a "
            "graffiti wall, a screen, a sign — without breaking the scene's recognition\n"
            "- movie_remix: the word integrates into the absurd alteration itself, "
            "reinforcing the visual joke\n\n"
            "Set text_element with the rendering technique and placement that matches "
            "your chosen direction. Set word_render.enabled to true."
        )
    return (
...
def _transition_prompt_block() -> str:
    ...
    return (
        ...
        "- tense tier verbs: staggers, surges forward, braces against, buffeted "
        "by, leans hard into, charges, drives hard, plants feet, recoils, "
        "pivots sharply, ducks, weaves, hauls on, grips tightly, strains "
        "against, shoulders into, whips around, snaps to, presses forward, "
        "holds firm under\n"
        "For tense scenes, avoid ballistic and unmoored language: catapults, "
        "launches, hurls, lurches, blasts, explodes (as a verb on the subject), "
        "shudders, collapses. These verbs encourage the video model to produce "
        "unphysical motion — vehicles leaving the ground, subjects deforming, "
        "objects detaching from their environment. Prefer grounded dynamic "
        "verbs (accelerates hard, drives fast, surges forward, brakes sharply) "
        "that convey energy without breaking physics. The scene should feel "
        "intense, not impossible.\n"
