...
AUDIO_VERSION = 1
SUPPORTED_TARGET_LANGUAGES = {"en", "ceb", "id", "de", "es", "fr", "ko", "pt"}
...
def _validate_scope(config: StaticTtsConfig) -> None:
    if config.target_language not in SUPPORTED_TARGET_LANGUAGES:
        raise RuntimeError("Only --target-language en, ceb, id, de, es, fr, ko, or pt is supported.")
...
def _target_language_aliases(target_language: str) -> set[str]:
    ...
    if target_language == "pt":
        return {"pt", "pt-br", "pt-pt", "portuguese", "português", "portugues"}
