...
AUDIO_VERSION = 1
SUPPORTED_TARGET_LANGUAGES = {"en", "ceb", "id", "de", "es", "fr", "ko", "pl"}
...
def _validate_scope(config: StaticTtsConfig) -> None:
    if config.target_language not in SUPPORTED_TARGET_LANGUAGES:
        raise RuntimeError("Only --target-language en, ceb, id, de, es, fr, ko, or pl is supported.")
...
def _target_language_aliases(target_language: str) -> set[str]:
    ...
    if target_language == "fr":
        return {"fr", "fr-fr", "french", "français", "francais"}
    if target_language == "ko":
        return {"ko", "ko-kr", "korean", "한국어", "hangul"}
    if target_language == "pl":
        return {"pl", "pl-pl", "polish", "polski", "język polski", "jezyk polski"}
