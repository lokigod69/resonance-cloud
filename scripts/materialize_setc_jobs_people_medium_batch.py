...
DEFAULT_BATCH_DIR = CURRICULUM_ROOT / "batch_runs" / "en" / "jobs_people_v1_1_contextual_role_portrait_2026_05_25_151305"
    ...
    for raw in (batch_dir / "batch_input.jsonl").read_text(encoding="utf-8").splitlines():
        ...
        records[custom_id] = {
            **meta,
            ...
            "set_role": "production_candidate_jobs_people_materialized",
...
def create_contact_sheet(records: list[dict[str, Any]], output_path: Path, title: str, columns: int = 5) -> None:
    ...
    draw.text((margin, 20), title, font=load_font(24, True), fill="#f4f7fb")
    for index, record in enumerate(records):
        ...
        draw.text((x, y + tile_h + 45), "jobs_people | candidate", font=load_font(10), fill="#7d8797")
...
def materialize(batch_dir: Path, output_dir: Path, status: dict[str, Any], api_key: str) -> dict[str, Any]:
    ...
    for raw in (batch_dir / DEFAULT_BATCH_OUTPUT_FILENAME).read_text(encoding="utf-8").splitlines():
        ...
        meta = {
            ...
            "dominant_visual_element": f"one dominant person clearly communicating {plan['word']} through role, context, tools, clothing, and posture",
    ...
    create_contact_sheet(output_records, output_dir / "contact_sheet.png", "Jobs & People v1.1 - medium batch")
    for level in sorted({record["level"] for record in output_records}):
        ...
        create_contact_sheet(level_records, output_dir / f"level_{level:02d}_contact_sheet.png", f"Jobs & People - Level {level}", columns=5)

    subroute_counts = Counter(record["subroute"] for record in output_records)
    report = [
        "# Jobs & People Medium Materialized Report",
        "",
        ...
        "## Review Expectations",
        "",
        "- Review contact sheets by level before any import decision.",
        "- Flag generic office portraits and repeated model faces.",
        "- Flag national-symbol dependency for legal/public roles.",
        "- Flag roles whose function is unclear without text.",
        "- Flag costume-only portraits with weak workplace context.",
    ]
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Poll and materialize the Jobs & People Set C medium OpenAI Batch output.")
