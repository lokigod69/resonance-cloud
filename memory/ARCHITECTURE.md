...
| Local DAW (legacy) | `src/app.py`, `src/routers/*`, `start*.bat` | FastAPI: workspace/words/generation/settings/suno/media routers + SPA static serving load only when `STORAGE_MODE != "cloud"`; cloud mode loads only health + imageless_tts. |
...
| Docs | `docs/` | `Stabilization/` (Phase 1A–1H + closeout), `architecture/`, `Infrastructure/` (incl. LTX video disable plan), `Product/` (guided Today focus), `Refactors/` (2026-07 cleanup audit). |
